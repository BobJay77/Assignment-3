...
std::vector<glm::vec3> EnvironmentGenerator::_spawnFromAll;
std::vector<glm::vec3> EnvironmentGenerator::_spawnToAll;
std::vector<std::vector<glm::vec3>> EnvironmentGenerator::_avoidFromAll;
std::vector<std::vector<glm::vec3>> EnvironmentGenerator::_avoidToAll;
		...
		std::vector<GameObject> temp;
		{
			...
			for (int j = 0; j < _numToSpawn[i]; j++)
			{
				...
				temp[j].get<Transform>().SetLocalPosition(glm::vec3(Util::GetRandomNumberBetween(_spawnFromAll[i],
					_spawnToAll[i], _avoidFromAll[i], _avoidToAll[i])));
...
void EnvironmentGenerator::AddObjectToGeneration(std::string fileName, ShaderMaterial::sptr objMat, int numToSpawn, glm::vec3 spawnFrom, 
													glm::vec3 spawnTo, std::vector<glm::vec3> avoidFrom, std::vector<glm::vec3> avoidTo)
{
