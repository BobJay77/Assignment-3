		...
		BackendHandler::imGuiCallbacks.push_back([&]() {
			if (ImGui::CollapsingHeader("Effect controls"))
			{
				...
				if (activeEffect == 4)
				{
					...
					PixelationEffect* temp = (PixelationEffect*)effects[activeEffect];
					float intensity = temp->GetIntensity();
				...
				if (activeEffect == 5)
				{
					ImGui::Text("Active Effect: Nightvision Effect");
...
// Load some textures from files
		Texture2D::sptr stone = Texture2D::LoadFromFile("images/grey.png");
		...
		TextureCubeMap::sptr environmentMap = TextureCubeMap::LoadFromImages("images/cubemaps/skybox/SpaceSky.png");
...
// We need to tell our scene system what extra component types we want to support
		...
		GameObject obj2 = scene->CreateEntity("Player Ship");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/PlayerShipBat.obj");
		...
		GameObject obj3 = scene->CreateEntity("Planet");
		{
			VertexArrayObject::sptr vao = ObjLoader::LoadFromFile("models/Yechin.obj");
			obj3.emplace<RendererComponent>().SetMesh(vao).SetMaterial(boxMat);
			obj3.get<Transform>().SetLocalScale(glm::vec3(0.1));
			obj3.get<Transform>().SetLocalPosition(50.0f, 0.0f, 10.0f);
			obj3.get<Transform>().SetLocalRotation(0.0f, 0.0f, -90.0f);
			BehaviourBinding::BindDisabled<SimpleMoveBehaviour>(obj3);
		}

		std::vector<glm::vec3> allAvoidAreasFrom = { glm::vec3(0.0f, 0.0f, 0.0f) };
		std::vector<glm::vec3> allAvoidAreasTo = { glm::vec3(0.0f, 0.0f, 0.0f) };

		std::vector<glm::vec3> rockAvoidAreasFrom = { glm::vec3(0.0f, 0.0f, 0.0f) };
		std::vector<glm::vec3> rockAvoidAreasTo = { glm::vec3(0.0f, 0.0f, 0.0f) };
		glm::vec3 spawnFromHere = glm::vec3(-50.0f, -50.0f, -50.0f);
		glm::vec3 spawnToHere = glm::vec3(50.0f, 50.0f, 50.0f);
		...
		EnvironmentGenerator::AddObjectToGeneration("models/simpleTree.obj", simpleFloraMat, 40,
			spawnFromHere, spawnToHere, allAvoidAreasFrom, allAvoidAreasTo);*/
		EnvironmentGenerator::AddObjectToGeneration("models/simpleRock.obj", simpleFloraMat, 500,
		...
		{
			cameraObject.get<Transform>().SetLocalPosition(0, 20, 20).LookAt(glm::vec3(0, 0, 10));
		...
		{
			geometryBuffer = &gBufferObject.emplace<GBuffer>();
			geometryBuffer->Init(width, height);
		}
		...
		{
			pixelationEffect = &pixelationEffectObject.emplace<PixelationEffect>();
			pixelationEffect->Init(width, height);
		}
		effects.push_back(pixelationEffect);

		GameObject nightvisionEffectObject = scene->CreateEntity("Nightvision Effect");
		{
...
#pragma endregion 
		...
		std::vector<KeyPressWatcher> keyToggles;
		{
			...
			keyToggles.emplace_back(GLFW_KEY_1, [&]() { drawCompositedScene = !drawCompositedScene; });
			keyToggles.emplace_back(GLFW_KEY_2, [&]() { drawPositionBuffer = !drawPositionBuffer; });
			keyToggles.emplace_back(GLFW_KEY_3, [&]() { drawNormalBuffer = !drawNormalBuffer; });
			keyToggles.emplace_back(GLFW_KEY_4, [&]() { drawColorBuffer = !drawColorBuffer; });
			keyToggles.emplace_back(GLFW_KEY_5, [&]() { drawLightAccumBuffer = !drawLightAccumBuffer; });
		...
		while (!glfwWindowShouldClose(BackendHandler::window)) {
			...
			illuminationBuffer->ApplyEffect(geometryBuffer);

			shadowBuffer->Unbind();

			if (drawGBuffer) {
				geometryBuffer->DrawBuffersToScreen();
			}
			...
			if (drawPositionBuffer) {
				geometryBuffer->DrawBufferToScreen('P');
			}
			if (drawNormalBuffer) {
				geometryBuffer->DrawBufferToScreen('N');
			}
			if (drawColorBuffer) {
				geometryBuffer->DrawBufferToScreen('A');
			}
			if (drawLightAccumBuffer) {
